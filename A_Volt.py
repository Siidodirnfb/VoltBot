...
ADMIN_IDS = {
    int(x) for x in os.environ.get("ADMIN_IDS", "7891780046").split(",") if x.strip().isdigit()
}
...
def copy_keyboard(loadstring: str) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        [[InlineKeyboardButton("Скопировать loadstring", copy_text=CopyTextButton(text=loadstring))]]
    )
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    await update.message.reply_text(
        "Привет. Команда /post запускает создание нового поста для канала "
        "𝙑𝙤𝙡𝙩 𝙃𝙪𝙗⚡︎.\n\nОтменить создание поста в любой момент — /cancel."
    )
...
async def get_game(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await update.message.reply_text(
        "Теперь отправьте функции одним сообщением — каждая функция с новой строки."
    )
    return FUNCTIONS
...
async def get_functions(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await update.message.reply_text("Введите ключ (State):")
    return KEY
...
async def get_key(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await update.message.reply_text(
        "Введите loadstring — этот текст будет копироваться по кнопке "
        "и нигде не будет показан в самом посте."
    )
    return LOADSTRING
